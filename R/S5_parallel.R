S5_parallel = function(NC,X,y,ind_fun,model,tuning,tem,ITER=20,S=20,C0=1){
  library(snowfall)
  library(Matrix)
  ...
  pmt=proc.time()
  wrapper  = function(i){
    fit = S5::S5(X=X,y=y,ind_fun=ind_fun, model = model,tuning=tuning,tem=tem,ITER=ITER,S=S,C0=C0,verbose=FALSE)
    return(fit)
  }
