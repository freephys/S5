SSS = function(X,y,ind_fun,model,tuning,N=1000,C0=1,verbose=TRUE){
  ...
  if(missing(ind_fun)){
    ...
    ind_fun = S5::ind_fun_pimom
    tuning <- S5::hyper_par(type="pimom",X,y,thre = p^-0.5)  # tuning parameter selection for nonlocal priors
  ...
  curr = ind_fun(sam) + model (ind2) # evaluate the objective value on the initial model
  ...
  for(uu in 1:C0){ # repeat the SSS algorithm C0 times  
    ...
    for(iter in 1:N){ 
      ...
      if(verb==TRUE&iter%%50==0){
        print("#################################")
        curr = ind_fun(ind2) + model(ind2) # print the objective value of the current model
